		            ...
		            <div class="block-27">
						<ul>
							...
							<c:forEach var="i" begin="${pageNo}" end="${pageNo + 9}">
								...
								<c:if test="${not doneLoop}">
									<c:choose>
					    				...
					    				<c:otherwise>
											<li><a href="/host/hostQnaList?currentPage=${i}&hostQnaCategory=${hostQnaCategory}">${i}</a></li>	
										</c:otherwise>		
									</c:choose>
							...
							<c:if test="${currentPage + 10 <= lastPage}">
								<li><a href="host/hostQnaList?currentPage=${pageNo+10}&hostQnaCategory=${hostQnaCategory}">&gt;</a></li>
							</c:if>
						</ul>
					</div>
				</div>
